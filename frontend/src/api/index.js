...
export const customerApi = {
  ...
  // file: a browser File object (the raw Aadhaar card photo). Uploaded
  // before the customer record exists yet - returns a Cloudinary url/
  // publicId that gets attached as the aadhaarPhoto field's value on the
  // subsequent create() call, same upload-first-attach-later pattern as
  // deliveryNoteApi.uploadVehiclePhoto / recordApi.uploadFile.
  uploadAadhaarScan: (file) => {
    const formData = new FormData();
    formData.append('aadhaarPhoto', file);
    return client
      .post('/customers/aadhaar-scan', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
      .then((r) => r.data);
  },
};
...
export const labourApi = {
  ...
  // files: an array of browser File objects (2-7 photos).
  uploadSiteLogPhotos: (site, date, files, location) => {
    const formData = new FormData();
