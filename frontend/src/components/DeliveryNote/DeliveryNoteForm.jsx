...
export default function DeliveryNoteForm() {
  ...
  const handleAadhaarScan = async (e) => {
    ...
    try {
      ...
      const result = await Tesseract.recognize(scannableFile, 'eng', {
        logger: (m) => {
          if (m.status === 'recognizing text' && typeof m.progress === 'number') {
            setAadhaarProgress(Math.round(m.progress * 100));
          }
        },
      });
      const { name, address } = extractAadhaarFields(result.data.text);
      if (!name && !address) {
        setAadhaarError('Could not read the card clearly. Please enter the details manually.');
        return;
      }
      // A standard Aadhaar card only prints the address on ONE side (usually
      // the back) - getting a name with no address most often just means the
      // front of the card was scanned, not that OCR failed. Say so, instead
      // of leaving Address blank with no explanation.
      if (name && !address) {
        setAadhaarError('Name found, but no address text on this photo - try scanning the other side of the card (the address is usually on the back), or enter it manually.');
      } else if (!name && address) {
        setAadhaarError('Address found, but no name text on this photo - try scanning the other side of the card, or enter it manually.');
      }
      setNewCustomer((prev) => ({
        ...prev,
        name: name || prev.name,
      ...
      setAadhaarError('Failed to scan the card. Please enter the details manually.');
    } finally {
  ...
  return (
    <div>
      ...
      <form className="panel form" onSubmit={handleSubmit}>
        ...
        {/* Preview of the captured/uploaded vehicle photo, shown neatly below the
            4-box row above rather than squeezed inside any one box. */}
            ...
            {useNewCustomer ? (
              <div className="form-grid">
                <div className="form-field">
                  ...
                  >
                    {scanningAadhaar
                      ? `⏳ Scanning card... ${aadhaarProgress}% (this can take up to 30s on some phones)`
                      : '📷 Scan Aadhaar'}
                  </button>
        ...
        <div className="form-grid" style={{ marginTop: '20px', marginBottom: '20px' }}>
          ...
          <div className="form-field">
            ...
            <input
              type="date"
              value={extraDate}
              onChange={(e) => handleTopFieldChange('dateTaken', e.target.value)}
            />
          </div>
          <div className="form-field">
            <label>Month</label>
            ...
            {items.map((it, idx) => (
              <tr key={idx}>
                ...
                <td data-label={t('deliveryForm.colDateReturned')}>
                  <input
                    type="date"
                    value={it.dateReturned}
                    onChange={(e) => updateItem(idx, 'dateReturned', e.target.value)}
                  />
                </td>
                {itemColumns.map((col) => (
        ...
        <div className="form-actions">
          ...
          {isEdit && (
            <button type="button" className="btn btn-ghost" onClick={() => navigate(`/billing/${id}/print`)}>
              {t('billing.print')}
            </button>
          )}
          {!isEdit && (
            <button type="button" className="btn btn-ghost" disabled={saving} onClick={handleSaveAndPrint}>
              {saving ? t('deliveryForm.saving') : t('billing.print')}
            </button>
          )}
