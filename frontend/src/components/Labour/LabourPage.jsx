...
function DailyEntryTab({ workers, loadingWorkers, sites, onPosted, t }) {
  ...
  // How long to keep sampling readings before settling on the best one, and
  // what accuracy (in meters) counts as "good enough to stop early" instead
  // of waiting out the full window - GPS-equipped phones outdoors often
  // lock a <20m fix well before the window ends, so there's no reason to
  // keep the person waiting once a clearly-good reading has come in.
  const LOCATION_SAMPLE_WINDOW_MS = 7000;
  const GOOD_ENOUGH_ACCURACY_M = 20;
  // Above this, warn rather than silently accept - a "location" this coarse
  // is realistically a WiFi/cell-tower estimate, not a real GPS fix, and
  // could be off by hundreds of meters to kilometers.
  const LOW_ACCURACY_WARNING_M = 100;
  ...
  const captureLocation = () => {
    ...
    const finish = () => {
      ...
      setLocating(false);
      if (!best) {
        setLogError('Could not get a location fix. Make sure Location Services are enabled and try again outdoors.');
        return;
      }
      const { lat, lng, accuracy } = best;
      setLogLocation({ lat, lng, accuracy, address: '' });
      setResolvingAddress(true);
      reverseGeocode(lat, lng)
        .then((address) => setLogLocation({ lat, lng, accuracy, address }))
    ...
    watchId = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        // A GPS fix typically improves over the first several readings as
        // more satellites lock in - keep whichever reading in this window
        // has the LOWEST (best) accuracy value seen so far, not just the
      ...
      (err) => {
        // A mid-window error (e.g. a transient signal drop) shouldn't
        // necessarily fail the whole capture if an earlier reading already
        // came in - only surface it as a hard failure if nothing usable was
        // ever received.
        if (!best) {
      ...
      // Real GPS fixes (as opposed to network/WiFi-based estimates) can
      // take longer than 10s to acquire indoors or in dense urban areas -
      // 20s gives it a realistic chance instead of silently falling back to
      // a coarse network estimate.
      { enableHighAccuracy: true, timeout: 20000, maximumAge: 0 }
    );

    setTimeout(finish, LOCATION_SAMPLE_WINDOW_MS);
  };
  ...
  const submitSiteLog = async () => {
    ...
    try {
      await labourApi.uploadSiteLogPhotos(logSite, logDate, logFiles, logLocation);
      setLogFiles([]);
  ...
  const shareEntriesViaWhatsApp = () =>
    shareSheetViaWhatsApp({
      ...
      message: `Name - Recent Labour Entries (${recentEntries.length} rows)`,
    ...
    <div>
      ...
      <div className="panel">
          ...
          <div className="form-field">
            ...
            {logLocation && !resolvingAddress && (
              <span className="field-hint">
                ...
                {typeof logLocation.accuracy === 'number' && (
                  <span className="stat-sub"> · accuracy ±{Math.round(logLocation.accuracy)}m</span>
                )}
              </span>
            )}
        ...
        <div style={{ marginTop: 16 }}>
          <h3 style={{ fontSize: '0.95em' }}>Recent Site Progress</h3>
          <table className="data-table stack-on-mobile">
            ...
            <tbody>
              {recentLogs.map((log) => (
                <tr key={log._id}>
                  <td data-label={t('labour.date')}>{formatDate(log.date)}</td>
                  <td data-label={t('labour.site')}>{log.site}</td>
                  ...
                  <td data-label="Photos">
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <div style={{ display: 'flex', gap: 4 }}>
                        {log.photos.slice(0, 3).map((p) => (
                          <a key={p.publicId} href={p.url} target="_blank" rel="noopener noreferrer">
                            <img
                              src={p.url}
                              alt={log.site}
                  ...
                  <td data-label="Logged By">{log.createdBy?.name || '-'}</td>
                </tr>
              ))}
              {recentLogs.length === 0 && (
                <tr>
                  <td colSpan={5} className="empty-row">
                    No site progress logged yet.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {!loadingEntries && !entriesError && (
          <table className="data-table stack-on-mobile">
            ...
            <tbody>
              {recentEntries.map((e) => (
                <tr key={e._id}>
                  <td data-label={t('labour.site')}>{e.site}</td>
                  <td data-label={t('labour.workerName')}>{e.workerName}</td>
                  <td data-label="No of person">{workerCountFor(e)}</td>
                  <td data-label={t('labour.wageEarned')}>{formatCurrency(e.wageEarned)}</td>
                  <td data-label="Old Balance">{formatCurrency(e.balanceBefore || 0)}</td>
                  <td data-label="Total">{formatCurrency(e.totalDue || e.wageEarned)}</td>
        ...
        {!loading && !error && (
          <table className="data-table stack-on-mobile">
            ...
            <tbody>
              {data?.rows.map((r) => (
                <tr key={r.workerId}>
                  <td data-label={t('labour.workerName')}>{r.workerName}</td>
                  <td data-label={t('labour.site')}>{r.site}</td>
                  <td data-label={t('labour.role')}>{r.role}</td>
                  <td data-label={t('labour.daysWorked')}>{r.daysWorked}</td>
                  <td data-label={t('labour.wageEarned')}>{formatCurrency(r.wageEarned)}</td>
                </tr>
              ))}
              {(!data || data.rows.length === 0) && (
                <tr>
