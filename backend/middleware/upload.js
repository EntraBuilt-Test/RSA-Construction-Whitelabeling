...
// Aadhaar card photo, captured during new-customer entry (before the
// customer record itself exists yet) - same limits/filter as the vehicle
// photo, different field name.
const uploadAadhaarPhoto = multer({
  storage,
  fileFilter,
  limits: { fileSize: 5 * 1024 * 1024, files: 1 },
}).single('aadhaarPhoto');

module.exports = { uploadVehiclePhoto, uploadReceiverPhoto, uploadAadhaarPhoto, uploadModuleFile, uploadSiteLogPhotos };
